...
function buildEvaluationPrompt(agent, rubric, normalizedTranscript, deterministicResults) {
  const system = [
    ...
    'Only populate fields that are directly applicable. Non-applicable values must be null; do not invent a promptPatch or actionAdjustment just to fill the field.',
    ...
    'Use the provided prompt only when deciding or drafting promptPatch. For evaluation and recommendations, rely on the rubric and observed evidence, not the provided prompt.',
      ...
      topRecommendations: [
        {
          ...
          priority: 1,
          promptPatch: 'Before offering an appointment, confirm: (1) property address, (2) whether the caller owns the property, and (3) selling timeline. Only then offer booking.',
          relatedRubricItems: ['rubric_item_id_1', 'rubric_item_id_2'],
          suggestedChange: 'Add a hard rule before the booking step: Do not offer or trigger appointment booking until property address, owner status, and selling timeline are confirmed.',
          title: 'short title',
        },
      ],
    }, null, 2),
